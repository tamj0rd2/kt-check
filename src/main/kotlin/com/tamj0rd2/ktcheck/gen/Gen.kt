...
sealed class Gen<T> {
    ...
    companion object {
        /**
         * Samples a value from the generator using the provided random instance.
         *
         * @param random The random instance used to create a seed for sampling. Defaults to [Random.Default].
         * @return A sampled value of type T.
         */
        fun <T> Gen<T>.sample(random: Random = Random.Default): T = samples(random).first()

        /**
         * Produces an infinite sequence of samples from the generator using the provided random instance.
         *
         ...
         */
        fun <T> Gen<T>.samples(random: Random = Random.Default) = generateSequence { sample(random.nextLong()) }
         ...
         * @return A sampled value of type T.
         */
        fun <T> Gen<T>.sample(seed: Long): T = generate(SampleTree.from(seed)).value
    }
}

/**
 * The result of generating a value from a generator, including the generated value and its shrinks.
 * Shrinks are represented as a sequence of [SampleTree]s, allowing for lazy evaluation and efficient traversal.
 */
internal data class GenResult<T>(val value: T, val shrinks: Sequence<SampleTree>)

private class CombinatorGenerator<T>(private val generator: (SampleTree) -> GenResult<T>) : Gen<T>() {
