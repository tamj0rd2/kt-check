...
class IntGeneratorTest {
    ...
    inner class Shrinking {
        ...
        fun `10 shrinks correctly`() {
            ...
            val (originalValue, shrinks) = gen.generateWithShrunkValues(StubValueProducer(listOf(10)))
        ...
        fun `shrinking zero produces no shrinks`() {
            ...
            val (originalValue, shrinks) = gen.generateWithShrunkValues(StubValueProducer(listOf(0)))
        ...
        fun `shrinks for non-zero numbers always include 0`() {
            generateSequence { Gen.int().generateWithShrunkValues() }
        ...
        fun `when 0 is in range, shrinks are closer to 0 than the original generated number`() {
            generateSequence { Gen.int(-50..50).generateWithShrunkValues() }
                .filter { (value) -> value != 0 }
                .take(100)
                .forEach { (value, shrinks) ->
